...
function findBuyerTransactions(userId){
    return new Promise((resolve, reject) => {
        try{
            resolve(Transaction.find({"buyer":userId}).populate("seller", ["username", "location", "phoneNumber"]).populate("productId"));
        } catch (err){
            reject(new Error(err));
        }
    })
...
function findSellerTransactions(userId){
    return new Promise((resolve, reject) => {
        try{
            resolve(Transaction.find({"seller":userId}).populate("buyer", ["username", "location", "phoneNumber"]));
        } catch (err){
            reject(new Error(err));
        }
    })
